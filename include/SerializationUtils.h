...
// The installed DBoW2 package doesn't add boost::serialization support to
// BowVector/FeatureVector (unlike the vendored fork), so provide it
// non-intrusively here since Atlas serialization needs it via KeyFrame.
namespace boost { namespace serialization {

template<class Archive>
void serialize(Archive &ar, DBoW2::BowVector &bv, const unsigned int)
{
    ar & boost::serialization::base_object<std::map<DBoW2::WordId, DBoW2::WordValue>>(bv);
}

template<class Archive>
void serialize(Archive &ar, DBoW2::FeatureVector &fv, const unsigned int)
{
