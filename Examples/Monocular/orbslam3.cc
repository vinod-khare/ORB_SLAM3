...
int    main(int argc, char **argv)
{
    try
    {
        ...
        desc.add_options()("help,h", "Show this help message")("vocab,v", po::value<string>()->required(), "Path to ORB vocabulary file")("settings,s", po::value<string>()->required(),
            ...
            "sequence,d", po::value<vector<string>>()->required()->multitoken(),
            "Sequence spec(s): image_dir [times_file]. If times_file is omitted, "
            "filename stems are used as timestamps")("timestamps-type", po::value<string>()->default_value("auto"), "Timestamps file format: auto, filename_ns, timestamp_ns, utc")(
        ...
        for (int seq = 0; seq < num_seq; seq++)
        {
            ...
            for (int ni = 0; ni < nImages[seq]; ni++, proccIm++)
            {
                ...
                const auto frame  = readers[seq].read();
                auto       im     = frame.image;
